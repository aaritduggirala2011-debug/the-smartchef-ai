...
import { addPantryItem, getPantry, removePantryItem } from "@/lib/store";
...
function Pantry() {
  ...
  const add = useMutation({
    mutationFn: async (name: string) => addPantryItem(name),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["pantry"] }),
    onError: (e: Error) => toast.error(e.message),
  });

  const remove = useMutation({
    mutationFn: async (id: string) => removePantryItem(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["pantry"] }),
  });
