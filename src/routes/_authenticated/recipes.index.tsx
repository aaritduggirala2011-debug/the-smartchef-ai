...
function RecipesPage() {
  ...
  const { data: pantry = [] } = useQuery({
    queryKey: ["pantry"],
    queryFn: async () => (await supabase.from("pantry_items").select("*")).data ?? [],
  });
  ...
  const { data: recipes = [], isLoading } = useQuery({
    queryKey: ["recipes"],
    queryFn: async () =>
      (await supabase.from("recipes").select("*").order("created_at", { ascending: false }).limit(24))
        .data ?? [],
  });
  ...
  const run = useMutation({
    mutationFn: async () => await generate({ data: { note: note.trim() || undefined } }),
    onSuccess: () => {
